            ...
            <tr>
                <td width="50%" valign="top">
                    <div class="bill-to">
                        ...
                        <p><strong>Company Name:</strong> {{ $order->company_name ?? 'N/A' }}</p>
                        <p><strong>Designation:</strong> {{ $order->designation ?? 'N/A' }}</p>
                        <p><strong>Company Phone:</strong> {{ $order->company_phone }}</p>
            ...
            <tbody>
                @foreach ($order->corporateOrderDetails as $index => $detail)
                    <tr>
                        ...
                        <td>
                            @php
                                $path = public_path($detail->packageProduct->image);
                                $imageData = file_exists($path) ? base64_encode(file_get_contents($path)) : null;
                                $mime = file_exists($path) ? mime_content_type($path) : null;
                            @endphp

                            @if ($imageData)
                                <img width="50" src="data:{{ $mime }};base64,{{ $imageData }}"
                                    alt="Product Image">
                            @else
                                <p>N/A</p>
                            @endif
