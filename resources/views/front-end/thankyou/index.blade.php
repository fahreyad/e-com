<x-guest-layout class="Thanks You">
    <section id="thankYouPage">
        <div class="container mx-auto p-8 pt-0 shadow-md rounded m-10">
            ...
            <section class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-gray-700">
                    <div><span class="font-semibold">Name:</span> {{ $order->name }}</div>
                    <div><span class="font-semibold">Phone:</span> {{ $order->phone }}</div>
                    <div class="md:col-span-2"><span class="font-semibold">Address:</span>
                        {{ $order->address ?? 'N/A' }}
                    </div>
                    ...
                    <div><span class="font-semibold">Delivery Area:</span>
                        {{ ucfirst(preg_replace('/([a-z])([A-Z])/', '$1 $2', $order->delivery_method)) }}</div>
                </div>
            </section>
                            ...
                            @foreach ($order->orderDetails as $index => $detail)
                                <tr class="hover:bg-gray-50">
                                    <td class="border border-gray-300 px-4 py-2">
                                        {{ $index + 1 }}</td>
                                    <td class="border border-gray-300 px-4 py-2">
                                        <img width="50" src="{{ $detail->product->image }}" alt="Product Image">
                                    </td>
                                    <td class="border border-gray-300 px-4 py-2">
                                        {{ $detail->product->name ?? 'Product not found' }}</td>
            ...
            <div class="mt-10 text-center text-gray-600">
                ...
                <a href="{{ route('products.index') }}"
                    class="inline-block mt-4 px-6 py-2 bg-[#FAAE43] text-white rounded  transition"><i
                        class="fa-solid fa-arrow-left"></i> Continue
                    Shopping</a>
            </div>
        </div>
    </section>
