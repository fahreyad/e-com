        ...
        <div class="w-full py-4 lg:w-1/4 lg:pr-4">
            <div class="w-full bg-white rounded">
                ...
                <table class="w-full text-sm">
                    <tr>
                        <td class="p-2 border-t border-b font-semibold">{{ __('Userame') }}</td>
                    ...
                    <tr>
                        <td class="p-2 border-t border-b font-semibold">{{ __('Phone') }}</td>
                        <td class="p-2 border-t border-b">{{ auth()->user()->phone }}</td>
                    </tr>
                    <tr>
                        <td class="p-2 border-t border-b font-semibold">{{ __('Address') }}</td>
                        <td class="p-2 border-t border-b">{{ auth()->user()->address }}</td>
                    </tr>           
            ...
            <div class="w-full bg-white rounded">
                ...
                <div class="w-full p-4">
                    <form action="{{ route('profile-update.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <x-labeled-input type="text" class="mt-4" name="address" value="{{ old('address', auth()->user()->address) }}" required/>
